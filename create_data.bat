...
echo Verificar,Data, Hora, Tamanho, Diretorio, Nome >> "%relatorio%"
...
for /f "delims=" %%f in ('dir /s /b /od "%dir%\*.avi"') do (
  ...
  if !filesize! equ 0 (
    set "Verificar=Erro"
    echo Corrompido: %%~dpf >> "%verificar_dados%"
    set /a "xcont=xcont+1"
  )
  ...
  if !cont! geq 1 if !hora! leq !horapos! if !hora! geq !horant! (
   if !minuto! geq !novosminutos! (
    if !hora! equ !novashoras! (
     ...
     echo Faltando Video >> "%relatorio%"
     set /a auth=1
     set /a "fcont=fcont+1"
     echo Caminho a verificar: %%~dpf >> "%verificar_dados%"    
    )
   )
  ...
  set /a auth=0
  echo !Verificar!, !ano!-!mm!-!dd!, !hh!:!mm!:!segundo!, !filesize!, %%~dpf, !fullname! >> "%relatorio%"
  
  set /a "cont=!cont!+1"
  
) 
  
echo Quantidade de videos faltando: !fcont! >> "%verificar_dados%"
echo Quantidade de videos corrompidos: !xcont! >> "%verificar_dados%"
